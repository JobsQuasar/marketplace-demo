...
module.exports = {
  ...
  workflows: {
    ...
    getStripeCustomer: {
      ...
      description: 'Create if there is none, or update existing Stripe customer',
      event: 'getStripeCustomer',
      context: ['stripe'],
      computed: {
        userId: 'objectId',
        user: 'object',
        stripeCustomer: '_.get(object, "platformData._private.stripeCustomer")',
      },
      run: [
        {
          name: 'stripeCustomer',
          stop: '!user',
          skip: '!!computed.stripeCustomer',
          endpointMethod: 'POST',
          endpointUri: '/integrations/stripe/request',
          endpointPayload: {
            method: '"customers.create"',
            args: {
              name: 'computed.user.displayName || ""',
        ...
        },
        {
          ...
          endpointPayload: {
            platformData: {
              _private: {
                stripeCustomer: 'responses.stripeCustomer'
              }
            }
          }
        },
        // TODO: update existing Stripe customer if there is one
        {
          endpointMethod: 'POST',
          endpointUri: '/signal',
          endpointPayload: {
            destination: 'computed.userId',
            event: '"getStripeCustomerSuccess"'
          }
        }
      ],
    },
    ...
    createStripeCheckoutSession: {
      ...
      description: 'A Stripe checkout session will be used in client-side',
      event: 'createStripeCheckoutSession',
      context: ['stripe', 'stelace'],
      computed: {
        // use of taker and asset from transaction related objects
        transactionId: 'objectId',
        transaction: 'object',
        stripeCustomer: '_.get(taker, "platformData._private.stripeCustomer")'
      },
      run: [
        {
          stop: '!computed.stripeCustomer || !asset',
          ...
          endpointPayload: {
            ...
            args: {
              ...
              customer: 'computed.stripeCustomer.id',
              success_url: '`${env.STELACE_INSTANT_WEBSITE_URL}/a/${computed.transaction.assetId}?payment-success=true&transactionId=${computed.transactionId}`',
              cancel_url: '`${env.STELACE_INSTANT_WEBSITE_URL}/a/${computed.transaction.assetId}?payment-cancel=true`',
              payment_method_types: ['"card"'],
              line_items: [
                {
                  name: 'asset.name',
                  description: 'asset.description',
                  amount: 'transaction.takerAmount * 100', // TODO: only works for currencies with 2 decimals
        ...
        },
        {
          ...
          endpointPayload: {
            message: {
              id: 'responses.checkoutSession.id'
            },
            destination: 'taker.id',
